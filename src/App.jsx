...
function App() {
  ...
  const [versiculoData, setVersiculoData] = useState({
    versiculo: "",
    localizacao: "",
  });
  ...
  function generateVersiculo() {
    const versiculoSize = QUOTEBANK.length;
    const randomIndex = Math.floor(Math.random() * versiculoSize);
    const randomVersiculoData = QUOTEBANK[randomIndex];
    setVersiculoData(randomVersiculoData);
    changeColor();
  }
  ...
  const xLink = `https://twitter.com/intent/tweet?hashtags=quotes&related=freecodecamp&text=%22${encodeURIComponent(
    versiculoData.versiculo
  )}%22%20-${encodeURIComponent(versiculoData.localizacao)}`;

  const handleCopyLink = () => {
    navigator.clipboard
      .writeText(`${versiculoData.versiculo} - ${versiculoData.localizacao}`)
      .then(() => {
        setShowNotification(true);
        setTimeout(() => setShowNotification(false), 3000); // Oculta a notificação após 3 segundos
      })
      .catch((err) => {
  ...
  return (
    <>
      {showNotification && (
        <div className="notification">Versículo copiado!</div>
      )}
      <Card
        versiculoData={versiculoData}
        generateVersiculo={generateVersiculo}
        xLink={xLink}
        Color={color}
